...
class EmotionChatMachine(object):
    def __init__(self, config_file, session, words2idx, word_embeddings, generic_word_length, start_id, end_id,
        ...
        self.entropy_loss = self.compute_loss()
        self.generation_words, self.pred_scores = self.generate()
        ...
        self.train = self.optimize(self.entropy_loss)
    ...
    @staticmethod
    def get_generation_symbols(predict_scores=None):
        symbols = tf.argmax(predict_scores, axis=1, output_type=tf.int32)
        return symbols
    ...
    def generate_response(self, last_encode_hiddens, last_encode_state):
        ...
        response_words = []
        likelihood_scores = []
        symbols = self.get_start_symbol()  # [batch, beam]
        response_words.append(symbols)  # [max len, batch, beam]
        with tf.variable_scope(self.name + "_decoder"):
            # first get top beam words
            start_symbol = [self.start_id for _ in range(self.batch_size)]
            this_input_embeddings = tf.nn.embedding_lookup(self.embeddings, start_symbol)  # [batch, embedding]
            this_emotion_attention = self.emotion_attention(last_encode_hiddens, output)
            this_input_vectors = tf.concat([this_input_embeddings, this_emotion_attention], axis=1)
            (output, state) = self.decode_cell(this_input_vectors, state)
            predict_scores = tf.sigmoid(tf.matmul(output, self.sfx_w) + self.sfx_b)  # [batch, total]
            predict_alpha = tf.sigmoid(tf.matmul(output, self.sfx_score_w))  # [batch, 1]
            final_predict_scores_smooth = tf.nn.softmax(self.get_unscaled_predict_scores(predict_scores, predict_alpha))
            values, symbols = tf.nn.top_k(final_predict_scores_smooth, k=self.beam_size)  # [batch, beam]
            likelihood_scores.append(values)  # [max len, batch, beam]
            response_words.append(symbols)  # [max len, batch, beam]
            outputs = [output for _ in range(self.beam_size)]  # [beam, batch, hidden]
            states = [state for _ in range(self.beam_size)]  # [beam, batch, state]
            symbols = tf.transpose(symbols)  # [beam, batch]

            # generate beam different path
            for i in range(1, self.config.max_len):
                tf.get_variable_scope().reuse_variables()

                this_symbols, this_states, this_outputs, this_scores = list(), list(), list(), list()
                for j in range(self.beam_size):
                    this_input_embeddings = tf.nn.embedding_lookup(self.embeddings, symbols[j])  # [batch, embedding]
                    this_emotion_attention = self.emotion_attention(last_encode_hiddens, outputs[j])
                    this_input_vectors = tf.concat([this_input_embeddings, this_emotion_attention], axis=1)
                    (output, state) = self.decode_cell(this_input_vectors, states[j])
                    this_states.append(state)
                    this_outputs.append(output)

                    predict_scores = tf.sigmoid(tf.matmul(output, self.sfx_w) + self.sfx_b)  # [batch, total]
                    predict_alpha = tf.sigmoid(tf.matmul(output, self.sfx_score_w))  # [batch, 1]
                    final_predict_scores_smooth = self.get_unscaled_predict_scores(predict_scores,
                                                                                   predict_alpha)  # [batch, total]
                    final_predict_scores_smooth = tf.nn.softmax(final_predict_scores_smooth)
                    symbol = self.get_generation_symbols(final_predict_scores_smooth)  # [batch]
                    score = tf.reduce_max(final_predict_scores_smooth, axis=1)  # [batch]
                    this_symbols.append(symbol)
                    this_scores.append(score)  # [beam, batch]

                symbols = this_symbols
                outputs = this_outputs
                states = this_states
                response_words.append(tf.transpose(symbols))  # [max len, batch, beam]
                likelihood_scores.append(tf.transpose(this_scores))  # [max len, batch, beam]
        response_words = tf.transpose(response_words, perm=[1, 2, 0])  # [batch, beam, max len]
        likelihood_scores = tf.reduce_sum(tf.transpose(likelihood_scores, perm=[1, 2, 0]), axis=-1)  # [batch, beam]
        return response_words, likelihood_scores
    ...
    def generate(self):
        last_encoder_hiddens, last_encode_state = self.encode()
        # [batch, beam, max len]  &  [batch, beam]
        generate_words, pred_scores = self.generate_response(last_encoder_hiddens, last_encode_state)
        return generate_words, pred_scores
    ...
    def train_step(self, this_input_x, this_post_len, this_input_y, this_predict_y, this_emotion_labels,
        ...
        output_feed = [self.train, self.entropy_loss]
        results = self.session.run(output_feed, input_feed)
        _, entropy_loss = results
        return entropy_loss
    ...
    def generate_step(self, this_input_x, this_post_len, this_emotion_labels, this_emotion_mask):
        ...
        output_feed = [self.generation_words, self.pred_scores, self.embeddings]
        results = self.session.run(output_feed, input_feed)
        words, pred_scores, embeddings = results[0], results[1], results[2]
        return words, pred_scores, embeddings
